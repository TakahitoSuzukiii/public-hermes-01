...
> ステータス: INFO / カテゴリ: GUIDE / 調査日 2026-08-12
> ⚠️ マスキング規約準拠。個人の機器固有情報（デバイスID等）は記載しない。
> 分類: Hermesエージェントへの音声・自然言語による指示方法に関する構築ドキュメント
> 関連: [20260812_INFO_PIXELWATCH_discord-voice-chat-feasibility.md](../info/research/20260812_INFO_PIXELWATCH_discord-voice-chat-feasibility.md)（Discordボイスチャット調査、本調査の前段）

## 0. 調査の目的

「Google Pixel Watch → 何らかの経路 → Hermesエージェント（Prime）」で音声から指示を出せるか。より抽象化すると「音声デバイスからHermesエージェントへ安全・安定して指示を出す現実的な方法」を精査する。

## 1. 結論（先出し）

**現在の構成（Discord経由でPrimeと会話）をそのまま使い、Pixel Watchの「通知クイック返信」機能を音声で使う方法が、追加アプリ・追加インフラ不要で最も現実的。**

```mermaid
flowchart LR
    A[Pixel Watch<br/>音声入力] -->|Wear OS標準:<br/>通知のクイック返信| B[Discordアプリの通知<br/>Reply欄<br/>スマホ側で動作]
    B -->|テキスト化して送信| C[Discordメッセージ送信<br/>鈴木さん→Primeのチャンネル]
    C --> D[Hermes/Prime<br/>が受信・応答]
```
...
5. 実際にPrimeから通知が来た際、Pixel Watch側に「返信」ボタン付きで通知が表示されるかを確認

## 4. 制約・注意点

| 項目 | 内容 |
|---|---|
| **応答の起点** | この経路は「Primeからの通知が来た時に、それに返信する」形。鈴木さんから**新規にゼロから話しかける**（通知なしの状態から）ケースには使えない可能性が高い（Wear OS側にDiscordのメッセージ作成専用UIがないため） |
...
1. **まず実機で試す**: PrimeからDiscordへ何かメッセージを送った後、Pixel Watch側に通知が来るか、返信ボタンがあるかを確認（コストゼロ・即実行可能）
